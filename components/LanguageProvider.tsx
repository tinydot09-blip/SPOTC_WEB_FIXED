...
const translateTree = (root: ParentNode, language: SpotcLanguage) => {
  ...
  for (const node of nodes) {
    ...
    /*
     * English mode must never rewrite React's live dynamic text.
     *
     * The previous observer cached the first value of a text node (for example
     * quantity "1"). When React updated that same node to "2", the observer ran
     * again and restored the cached "1".
     *
     * In English there is nothing to translate, so keep the current React value
     * and refresh the stored source instead.
     */
    if (language === 'en') {
