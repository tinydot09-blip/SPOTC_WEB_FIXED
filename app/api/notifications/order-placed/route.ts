...
function orderNumber(id: string, data: FirebaseFirestore.DocumentData): string {
  return text(data.order_number) || text(data.order_id) || id;
}

function orderTotal(data: FirebaseFirestore.DocumentData): number {
  return numberValue(
    data.total ?? data.grand_total ?? data.total_amount ?? data.amount ?? 0,
  );
}
...
async function sendPush(
  ...
  try {
    ...
    console.error('[SPOTC] Order placed push failed:', error);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!orderSnap.exists) {
      return NextResponse.json({ ok: false, error: 'Order was not found.' }, { status: 404 });
    }
    ...
    if (!uid || uid !== decoded.uid) {
      return NextResponse.json(
        { ok: false, error: 'This order does not belong to you.' },
        { status: 403 },
      );
    }
    ...
    const customerTitle = 'SPOTC — Order Placed';
    const customerBody = `We received your order #${number}.`;
    ...
    const adminBody = `Order #${number} received • ₹${Math.round(total)}`;
    ...
    await adminDb.collection('AdminNotifications').doc().set({
      ...
      title: adminTitle,
      body: adminBody,
      url: `/admin/orders?search=${encodeURIComponent(number)}`,
      is_read: false,
      created_at: FieldValue.serverTimestamp(),
    });
    ...
    console.error('[SPOTC] Order placed notification failed:', error);
    return NextResponse.json(
      {
        ok: false,
